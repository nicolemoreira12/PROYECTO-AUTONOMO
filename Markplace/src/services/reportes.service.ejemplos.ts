...
async function ejemploEstadisticasCategorias() {
  try {
    ...
    const mejorTicket = estadisticas.reduce((prev, current) => 
      (current as any).ticketPromedio > (prev as any).ticketPromedio ? current : prev
    );
...
async function ejemploInventarioPorEstado() {
  try {
    const productosAgotados = await reportesService.obtenerReporteInventario(
      1, 50, undefined, undefined, 'Agotado'
    );
...
async function ejemploReporteOrdenes() {
  try {
    const ordenes = await reportesService.obtenerReporteOrdenes(
      1, 20, '2024-01-01', '2024-12-31', 'Completado'
    );
...
async function ejemploCompararPeriodos() {
  try {
    const comparacion = await reportesService.compararPeriodos(
      '2024-01-01', '2024-06-30', // Primer semestre
...
async function ejemploValidacionFechas() {
  try {
    ...
    await reportesService.obtenerReporteIngresos(
      '2024-12-31',
      '2024-01-01',
      'mes'
    );
